//==============================================================================
// Instruction Cache - Direct-Mapped, Read-Only, Synchronous Read
// 
// Pipeline:
//   Cycle 0: fetch_addr registered into addr_r
//   Cycle 1: index extracted, arrays read → data_r available
//   Cycle 2: hit computed, data valid at output
//
//   Hit latency: 2 cycles from fetch_addr to fetch_data
//   Miss: stalls pipeline, fills line, then responds
//==============================================================================
...
module icache (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            data_valid_r <= addr_valid_r && (state != CACHE_MISS_WAIT || bus_resp_valid);
        end
    end
    ...
    always_comb begin
        ...
        case (state)
            CACHE_IDLE: begin
                ...
                // Miss: valid address in stage 1, tag mismatch detected in stage 2
                // We detect miss when hit_r is computed (stage 2) and it's a miss
                else if (addr_valid_r && data_valid_r && !hit_r && !inval_valid) begin
    ...
    //==========================================================================
    // Miss Address Capture (when miss detected in stage 2)
    //==========================================================================
    always_ff @(posedge clk) begin
        if (state == CACHE_IDLE && addr_valid_r && data_valid_r && !hit_r && !inval_valid) begin
            miss_addr_r  <= addr_r;
