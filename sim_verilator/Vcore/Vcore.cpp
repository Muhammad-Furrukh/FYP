...
Vcore::Vcore(VerilatedContext* _vcontextp__, const char* _vcname__)
    ...
    , clk{vlSymsp->TOP.clk}
    , clk_m{vlSymsp->TOP.clk_m}
    , rst{vlSymsp->TOP.rst}
    , rst_m{vlSymsp->TOP.rst_m}
    , rootp{&(vlSymsp->TOP)}
{
...
VL_ATTR_COLD void Vcore::trace(VerilatedFstC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vcore::trace()' shall not be called after 'VerilatedFstC::open()'.");
    }
